...
def getjobID_num(sbatch_response):  # subprocess.Popenproc，sbatchjpobID
    import re
...
def initDesignMatrix(numberOfTRs=1,
                     ignoreGreySquaresTrials=True):  # trial，ignoreGreySquaresTrials=True
...
def initDesignMatrix_newDesign(numberOfTRs=1, ignoreGreySquaresTrials=True):
    if ignoreGreySquaresTrials:
        designMatrixConditions = []
        for img in range(1, 17):
            designMatrixConditions.append(f"img{img}")
        for img in range(1, 17):
            designMatrixConditions.append(f"img{img}_grey")
        for condition in designMatrixConditions:
            print(condition, end=' ')
    else:
        designMatrixConditions = []
...
# def getDesignMatrix(
#         behav):  # : runcondition。[1-1；1-2；1-3；1-4；1-5；21,......]
#     """design matrix conditionrun，，trial，：
...
#     ImgTR = TRimgList != 0
#     trialList = TRimgList[ImgTR]  #  trial ，？ in the current trial, which image is shown?
#     greySquareTrial = np.asarray(
#         behav['Change'])  #  trial ，？ in the current trial, is there a grey square?
...
#             else:
#                 timesAppeared.loc[0, img] = timesAppeared.loc[0, img] + 1
#                 times = timesAppeared.loc[0, img]  # {img}? timesAppeared.loc[0, img]0，times=1。
#                 designMatrix.loc[currTR, f"img{img}_{times}"] = 1
#     # % matplotlib inline
#     # plt.imshow(np.asarray(designMatrix).astype(int))
#     assert designMatrix.shape == (
#         numberOfTRs, 16 * (5 + 5))  # 165，grey（80trial10grey）。，5grey，5+5
...
#     try:
#         assert designMatrix.shape == (numberOfTRs, 16 * 5)  # 80trial
#     except:
#         try:
#             assert designMatrix.shape == (
#                 numberOfTRs, 77)  # sub019 637trial，640=80*8runtrial，5run77trial
#         except:
...
def getDesignMatrix(
        behav):  # ，glmsingle，。MRMD-AE，run，conditionimg1 img2 ... img16 grey
    ...
    # timesAppeared.loc[0, :] = 0
    # designMatrix = initDesignMatrix(numberOfTRs=numberOfTRs, ignoreGreySquaresTrials=True)
    designMatrix = initDesignMatrix_newDesign(numberOfTRs=numberOfTRs, ignoreGreySquaresTrials=True)
    ...
    for currTR_1, img in enumerate(TRimgList):
        ...
        if img != 0:
            ...
            if greySquareTrial[currTR_1] == 1:
                # timesAppeared.loc[0, -img] = timesAppeared.loc[0, -img] + 1
                # times = timesAppeared.loc[0, -img]
                designMatrix.loc[currTR, f"img{img}_grey"] = 1
            else:
                # timesAppeared.loc[0, img] = timesAppeared.loc[0, img] + 1
                # times = timesAppeared.loc[0, img]  # {img}? timesAppeared.loc[0, img]0，times=1。
                designMatrix.loc[currTR, f"img{img}"] = 1
    # % matplotlib inline
    # plt.imshow(np.asarray(designMatrix).astype(int))
    assert designMatrix.shape == (
        numberOfTRs, 16*2)  # 165，grey（80trial10grey）。，5grey，5+5
    # print(f"timesAppeared={timesAppeared}")
    ...
    # designMatrix = deleteZeroColumn(designMatrix)
    print(f"designMatrix.shape={designMatrix.shape}")
    print(f"how many trials are there = np.sum(np.asarray(designMatrix))={np.sum(np.asarray(designMatrix))}")
    # try:
    #     assert designMatrix.shape == (numberOfTRs, 16 * 5)  # 80trial
    # except:
    #     try:
...
def normalize(X, axis=0):
    ...
    _X[np.isnan(_X)] = 0  # voxel（voxel）zscorenan，nan 0 。
    return _X
...
def normalize_FilteredFunc_T1(sub='', run=0):
    ...
    if not os.path.exists(filteredFunc_T1_norm):
        cmd = f"flirt -ref {example_funcInhighres} -in {filteredFunc} -out {filteredFunc_T1} -init {func2T1} -applyisoxfm 1.5"  # -applyxfm -applyxfm filteredFunc1.5mm，。
        kp_run(cmd)
        filteredFunc_T1_data = nib.load(filteredFunc_T1)  # filteredFunc 128x128x90x250  filteredFunc_T1 138x170x170x250
        ...
        assert filteredFunc_T1_data.shape[0:3] == (138, 170, 170)  # 4，。
        filteredFunc_T1_norm_data = normalize(filteredFunc_T1_data, axis=3)  # 
...
def loadBrainData(sub='', run=0):
    ...
    assert brain.shape[1:] == (138, 170, 170)  # 1，。

    # 3TR 4.5s？FSL feat4.5s3TR？
    #     ？HRF，，HRFlag。。
    # brain = brain[3:, :]  # ， align TR for functional brain data and behavior data.
...
jobarrayID = int(float(sys.argv[1]))  # 11
...
def getBrainBehav(sub='', run=1):
    ...
    MRMD_AE = False
    if MRMD_AE:
        designMatrix = getDesignMatrix_MRMD_AE(behav)
    else:
        designMatrix = getDesignMatrix(behav)
    # designMatrix = getDesignMatrix(behav)

    brain = np.transpose(brain, (1, 2, 3, 0))  # brain[:,:,:,TR]
...
for run in tqdm(range(1, 1 + runNum)):
    ...
    assert _designMatrix_.shape == (brain.shape[3], 16*2)
    # if not sub == 'sub019':
    ...
    # else:  # sub019，5run77trial，1-4run，condition-3 ；
    #     # 5run，；6-8run，condition-3。
    #     if run == 5:
    #         _designMatrix_wide = np.concatenate([np.zeros([_designMatrix_.shape[0], 80 * (run - 1)]),
    #                                              _designMatrix_,
    #                                              np.zeros([_designMatrix_.shape[0], 80 * (runNum - run)])], axis=1)
    ...
    if testMode:
        ...
        # brain = brain[(t - 30):(t + 30), :, :, :]  # brain [:,:,:,TR]  brain.shape=(138, 170, 170, 247)
        t = int(brain.shape[2] / 2)
        brain = brain[:, :, (t - 10):(t + 10), :]  # brain [:,:,:,TR]  brain.shape=(138, 170, 170, 247)
    brains.append(brain)
    designMatrixsDataFrames[run] = designMatrix
    conditionRecords[run] = list(designMatrix.columns)
    designMatrixs.append(_designMatrix_)
...
opt['wantlibrary'] = 1  # HRF
opt['wantglmdenoise'] = 1  # GLMdenoise
opt['wantfracridge'] = 1  # ridgeβ

# ，，。 For the purpose of this example we will keep the relevant outputs in memory and also save them to the disk
# wantfileoutputs[A B C D]，（）。
# A = 0/1 ONOFF ，
# B = 0/1 FITHRF ，
# C = 0/1 FITHRF_GLMdenoise ，
# D = 0/1 FITHRF_GLMdenoise_RR 。
# [1 1 1 1] 。
# opt['wantfileoutputs'] = [1, 1, 1, 1]
opt['wantfileoutputs'] = [0, 0, 0, 0]  # numpypickle4GB。h5py。

# wantmemoryoutputs[A B C D]，<results>。[0 0 0 1]D。
...
def visualize(sub=''):  # load existing file outputs if they exist
    ...
    for i in range(9):
        plt.subplot(3, 3, i + 1)
        plt.imshow(beta_mean[:, :, i*2+2].T, cmap='RdBu_r', clim=[-2e5,2e5]) #, clim=[-3e7, 3e7] , clim=[-3e4,3e4]
        plt.colorbar()
        # plt.title(titlestr)
        plt.axis(False)
    # for i in range(9):
    ...
    for i in range(9):
        plt.subplot(3, 3, i + 1)
        plt.imshow(R2[:, :, i*2], cmap='copper', clim=[0, 80])  #, clim=[0, 10]
        plt.colorbar()
        # plt.title(titlestr)
        plt.axis(False)
