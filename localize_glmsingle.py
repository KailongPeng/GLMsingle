...
def getDesignMatrix(
        behav):  # 在这一个新的设计矩阵中，由于我意识到glmsingle必须要有重复，而不是之前那种毫无重复的。因此对于这一个不是为了MRMD-AE设计的设计矩阵，对于所有的run，condition一律为img1 img2 ... img16 grey
    ...
    # plt.imshow(np.asarray(designMatrix).astype(int))
    assert designMatrix.shape == (
        numberOfTRs, 16*2)  # 其中16张图片展示了5次，有时候有grey（80个trial中有10个grey）。对每张图片来说，最多有5个grey，所以是5+5
    # print(f"timesAppeared={timesAppeared}")
...
def loadBrainData(sub='', run=0):
    ...
    # 是否应该取消对3个TR 4.5s的补偿？好像FSL feat中压根没有说明4.5s或者3TR？
    #     为什么呢？因为使用了HRF，因此，HRF的lag被自动解释了。经过实验之后发现确实应该取消。
...
for run in tqdm(range(1, 1 + runNum)):
    ...
    if testMode:
        ...
        # brain = brain[(t - 30):(t + 30), :, :, :]  # brain的维度为 [:,:,:,TR]  brain.shape=(138, 170, 170, 247)
        t = int(brain.shape[2] / 2)
        brain = brain[:, :, (t - 10):(t + 10), :]  # brain的维度为 [:,:,:,TR]  brain.shape=(138, 170, 170, 247)
    brains.append(brain)
